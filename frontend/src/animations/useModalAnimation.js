...
export const useModalAnimation = (isOpen, { overlayRef, modalRef, onCloseComplete, config = {} }) => {
  ...
  useEffect(() => {
    ...
    if (!overlayRef.current || !modalRef.current) {
      ...
      setTimeout(() => setIsRendered(false), 0)
  ...
  useEffect(() => {
    ...
    if (isUncontrolled || isOpen) {
      ...
      return () => {
        ...
        gsap.killTweensOf([overlayEl, containerEl])
      }
    }
    ...
    tl.to(containerEl, { scale: 0.92, opacity: 0, duration: 0.25, ease: 'power2.in' })
    tl.to(overlayEl, { opacity: 0, duration: 0.2, ease: 'power2.in' }, '-=0.1')

...
const VerifyEmail = () => {
  ...
  const handlePaste = (e: ClipboardEvent<HTMLInputElement>) => {
    ...
    if (pastedCode.some((char: string) => isNaN(Number(char)))) return

    const newCode = [...code]
    pastedCode.forEach((char: string, index: number) => {
      if (index < 6) {
        newCode[index] = char
      }
    })
    ...
    if (pastedCode.length === 6) {
      setTimeout(() => {
        void verifyEmail(pastedCode.join(''))
          .then(() => {
            setIsVerified(true)
      ...
      <div className="auth-stagger w-full max-w-[400px] bg-black/40 backdrop-blur-2xl border border-white/[0.08] rounded-[32px] p-8 sm:p-10 shadow-[0_0_40px_rgba(0,0,0,0.5)]">
        {!isVerified ? (
          <>
            ...
            <div className="auth-stagger mt-8 text-center">
                <p className="text-sm text-text-dim">
                  ...
                  >
                    {isResendingVerificationEmail ? <Spinner size="sm" /> : 'Resend'}
                  </button>
                </p>
            </div>
          </>
          ...
          <div className="success-animation text-center py-4">
            ...
            <p className="text-text-dim text-sm mb-4">Your account has been successfully verified. Redirecting...</p>
          </div>
        )}
      </div>
    </div>

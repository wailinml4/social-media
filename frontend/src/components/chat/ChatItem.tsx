...
import { useHoverBackground } from '../../animations/useHoverScale.js'
...
const ChatItem = ({ chat, isActive, onClick, isCollapsed }: ChatItemProps) => {
  ...
  return (
    <div
      ref={itemRef}
      onClick={onClick}
      onMouseEnter={e => !isActive && handleMouseEnter(e.currentTarget)}
      onMouseLeave={e => !isActive && handleMouseLeave(e.currentTarget)}
    ...
    >
      {isActive && <div className="absolute left-0 top-0 bottom-0 w-1 bg-primary shadow-[0_0_10px_rgba(10,132,255,0.4)]" />}

      <div className="relative flex-shrink-0">
        <img src={chat.user.avatar} alt={chat.user.name} className="w-12 h-12 rounded-full border border-white/10" />
        {chat.user.online && <div className="absolute bottom-0 right-0 w-3.5 h-3.5 bg-green-500 border-2 border-black rounded-full" />}
      </div>
      ...
      {!isCollapsed && (
        <div className="flex-1 min-w-0 transition-opacity duration-300">
          ...
          <div className="flex justify-between items-center">
            <p className={`text-sm truncate ${chat.unread && chat.unread > 0 ? 'text-white font-medium' : 'text-text-dim'}`}>
              {chat.lastMessage}
            </p>
            {(chat.unread ?? 0) > 0 && (
              <span className="bg-primary text-white text-[10px] font-bold px-2 py-0.5 rounded-full ml-2">{chat.unread}</span>
            )}
          </div>
        </div>
      )}

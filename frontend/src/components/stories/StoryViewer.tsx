...
import StoryProgressBar from './StoryProgressBar.jsx'
import { useModal } from '../../context/ModalContext.jsx'
import { setStoryResumeSlideIndex } from '../../services/storyService.js'
import defaultAvatar from '../../assets/default-avatar.svg'

const DEFAULT_STORY_DURATION = 5000

const formatRelativeTimestamp = (timestamp: string | Date | undefined, fallbackDate: string | Date | undefined) => {
...
interface StoryViewerProps {
  ...
  canDelete?: boolean
  onDelete?: (storyId: string, index: number) => void
  onStoryChange?: (index: number) => void
  onClose?: (index: number) => void
  onComplete?: (storyId: string, index: number) => void
}
...
const StoryViewer = ({
  ...
  useEffect(() => {
    ...
    tl.fromTo(containerRef.current, { scale: 0.92, opacity: 0 }, { scale: 1, opacity: 1, duration: 0.35, ease: 'power3.out' }, '-=0.15')
    return () => {
      tl.kill()
    }
  }, [])
  ...
  useEffect(() => {
    ...
    setTimeout(() => setSlideIndex(Math.min(startSlideIndex, stories[startIndex]?.slides.length - 1 || 0)), 0)
  ...
  const handleClose = useCallback(() => {
    if (progressInterval.current !== null) clearInterval(progressInterval.current)
    const tl = gsap.timeline({
      onComplete: () => {
        if (slideIndex === totalSlides - 1 && stories[storyIndex]?.id) {
          onComplete?.(stories[storyIndex].id, storyIndex)
        }
        onClose?.(storyIndex)
      },
    })
    if (containerRef.current) tl.to(containerRef.current, { scale: 0.92, opacity: 0, duration: 0.25, ease: 'power2.in' })
    if (overlayRef.current) tl.to(overlayRef.current, { opacity: 0, duration: 0.2, ease: 'power2.in' }, '-=0.1')
  ...
  const goNext = useCallback(() => {
    if (slideIndex < totalSlides - 1) {
      ...
      setSlideIndex(i => i + 1)
    } else if (storyIndex < stories.length - 1 && stories[storyIndex]?.id) {
      onComplete?.(stories[storyIndex].id, storyIndex)
      setCurrentProgress(0)
      setStoryIndex(i => i + 1)
      setSlideIndex(0)
    } else if (stories[storyIndex]?.id) {
      onComplete?.(stories[storyIndex].id, storyIndex)
      onClose?.(storyIndex)
    }
  ...
  const startProgress = useCallback(() => {
    if (progressInterval.current !== null) clearInterval(progressInterval.current)
    progressStart.current = Date.now() - (elapsed.current || 0)

    progressInterval.current = setInterval(() => {
      if (isHolding.current) return
      const delta = Date.now() - (progressStart.current || 0)
  ...
  const handleDeleteSlide = useCallback(async () => {
    ...
    openConfirmModal({
      ...
      onConfirm: async () => {
        setIsDeleting(true)
        try {
          await onDelete(currentSlide.id || currentSlide._id || '', slideIndex)
        } finally {
          setIsDeleting(false)
        }
      },
    })
  }, [canDelete, currentSlide, onDelete, openConfirmModal, slideIndex])
      ...
      >
        ...
        >
          ...
          <div className="absolute top-0 inset-x-0 px-4 pt-4 z-30">
            <StoryProgressBar slides={currentStory.slides} currentIndex={slideIndex} currentProgress={currentProgress} />
              ...
              <div className="flex items-center gap-2.5">
                <div
                  ...
                  onClick={(e: React.MouseEvent) => {
                    e.stopPropagation()
                    try {
                      const uid = currentStory.user?._id || currentStory.user?.id || currentStory.user?.uuid || null
                ...
                >
                  <img
                    src={currentStory.user?.avatar || currentStory.user?.profilePicture || defaultAvatar}
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-white/50 text-[11px] leading-tight">
                    {String(formatRelativeTimestamp(currentSlide.timestamp, currentStory.createdAt) || '')}
                  </span>
                </div>
              </div>
          ...
          {currentSlide.caption && (
            <div className="absolute bottom-8 inset-x-0 px-5 z-10 pointer-events-none">
              <p className="text-white text-sm font-medium leading-snug drop-shadow-sm">{currentSlide.caption}</p>
            </div>
          )}

          <button className="absolute left-0 top-0 bottom-0 w-1/3 z-20" onClick={goPrev} aria-label="Previous" />
          <button className="absolute right-0 top-0 bottom-0 w-1/3 z-20" onClick={goNext} aria-label="Next" />
        </div>

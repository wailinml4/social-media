...
type FollowButtonVariant = 'primary' | 'default'
...
const FollowButton = ({ isFollowing, isLoading, onFollow, variant = 'primary', size = 'sm' }: FollowButtonProps) => {
  const sizeClasses: Record<FollowButtonSize, string> = {
    sm: 'px-3 py-1.5 text-xs',
    md: 'px-4 py-2 text-sm',
  }

  const iconSizes: Record<FollowButtonSize, string> = {
    sm: 'w-3 h-3',
    md: 'w-4 h-4',
  }

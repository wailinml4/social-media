...
import type { ConfirmModalData } from '../../types/ui.js'
...
const ConfirmModal = () => {
  ...
  const { title = 'Confirm', message = 'Are you sure?', confirmText = 'Confirm', cancelText = 'Cancel', onConfirm } = confirmModalData
  ...
  return (
    <div className="fixed inset-0 z-[140] flex items-end justify-center sm:items-center sm:p-6">
      <div ref={overlayRef} className="absolute inset-0 bg-black/40 backdrop-blur-xl" onClick={closeConfirmModal} />
      ...
      >
        <div className="flex items-center justify-between border-b border-white/10 px-5 py-4">
          <div>
            ...
            {confirmModalData.subtitle && <p className="mt-1 text-sm text-text-dim">{confirmModalData.subtitle}</p>}
          </div>
          <button
            type="button"

...
const PostAuthor = ({ author, time, onTimeClick, showDropdown = false, dropdownContent }: PostAuthorProps) => {
  ...
  return (
    <div className="flex items-center justify-between gap-3 mb-4">
      <div className="flex items-center gap-3 min-w-0">
        <Avatar src={author.profilePicture || author.avatar} alt={authorName} name={authorName} size="md" to={`/profile/${authorId}`} />
        <div className="min-w-0">
          <Link to={`/profile/${authorId}`} className="block text-base font-semibold text-white hover:underline truncate">
            {authorName}
          </Link>

  ...
  return (
    ...
    >
      <input
        ...
        accept="image/*,video/*"
        className="hidden"
        onChange={(e: ChangeEvent<HTMLInputElement>) => onAddFiles(e.target.files || new DataTransfer().files)}
      />
      ...
      {mediaItems.length === 0 ? (
        ...
        <div className="grid grid-cols-3 gap-2 p-2">
          {mediaItems.map(item => (
            <div key={item.id} className="group relative aspect-square overflow-hidden rounded-2xl bg-white/6">
              {item.type === 'image' ? (
                <img
                  src={item.preview}
                  alt={item.file.name}

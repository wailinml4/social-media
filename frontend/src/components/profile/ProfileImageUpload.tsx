...
interface ProfileImageUploadProps {
  ...
  coverSrc?: string
  userName: string
  onAvatarChange: (files: ProcessedFile[]) => void
  onCoverChange: (files: ProcessedFile[]) => void
}

const ProfileImageUpload = ({ avatarSrc, coverSrc, userName, onAvatarChange, onCoverChange }: ProfileImageUploadProps) => {
  return (
    <>
      {/* Cover Image */}
      <div className="relative mb-16 h-32 overflow-hidden rounded-xl bg-white/5 sm:h-40">
        ...
        <FileUploader accept="image/*" onFilesChange={onCoverChange} className="absolute right-3 top-3">
          <button
            type="button"
      ...
      <div className="absolute left-4 top-24 sm:left-5 sm:top-28">
        <div className="relative">
          <Avatar src={avatarSrc} alt="Avatar" name={userName} size="xl" className="h-24 w-24 sm:h-28 sm:w-28 border-4 border-[#050505]" />
          <FileUploader accept="image/*" onFilesChange={onAvatarChange} className="absolute bottom-1 right-1">
            <button
              type="button"

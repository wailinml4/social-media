...
const ProfileStats = ({ stats }: ProfileStatsProps) => {
  ...
  return (
    <div className="grid grid-cols-4 gap-3 md:gap-4 mb-8 px-4 md:px-6 mt-6">
      {statItems.map(item => (
        <div
          key={item.key}
          onClick={() => item.key !== 'posts' && handleClick(item.key)}
          className={`stat-card-anim flex flex-col items-center justify-center p-3 md:p-4 rounded-2xl bg-white/5 border border-white/10 hover:border-white/20 hover:bg-white/10 transition-all duration-300 group backdrop-blur-sm ${
        ...
        >
          ...
          <span className="text-xs md:text-sm text-gray-400 font-medium tracking-wide mt-1">{item.label}</span>
        </div>
      ))}
    </div>
  )

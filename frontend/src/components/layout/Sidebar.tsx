...
import { Bell, Bookmark, Home, MessageCircle, MoreHorizontal, Plus, User, Zap, LogOut, Compass } from 'lucide-react'
...
// ── Individual nav item ───────────────────────────────────────────────────────
const SidebarItem = ({ to, icon, label, active, onClick, fillWhenActive = true, badge = 0 }: SidebarItemProps) => {
  const IconComp = icon
  const itemClassName = `sidebar-item flex items-center gap-3 px-4 py-3.5 rounded-3xl transition-all duration-200 group w-full ${
    active ? 'text-white bg-white/[0.05]' : 'text-white/40 hover:text-white/80 hover:bg-white/[0.04]'
  }`
  ...
  const content = (
    <>
      <div className="relative shrink-0">
        ...
        />
        {active && <span className="absolute -right-1 -top-1 w-1.5 h-1.5 rounded-full bg-primary" />}
        {badge > 0 && (
      ...
      <span className={`sidebar-label whitespace-nowrap text-[15px] overflow-hidden ${active ? 'font-semibold' : 'font-normal'}`}>
        {label}
      </span>
    </>
  )
      ...
      <div className="h-full w-full px-4 py-5">
        <div className="flex h-full w-full flex-col rounded-[36px] border border-white/10 bg-[#050505]/90 shadow-[0_24px_80px_rgba(0,0,0,0.27)] backdrop-blur-xl px-4 py-5 gap-5">
          ...
          >
            ...
            <span className="sidebar-label text-[20px] font-black tracking-tight text-white whitespace-nowrap overflow-hidden">Nexus</span>
          </Link>

          {/* Nav */}
          <nav className="flex-1 flex flex-col gap-3 w-full">
            {NAV_ITEMS.map(({ to, icon, label }: NavItem) => (
              <SidebarItem
                key={to}
                to={to}
                icon={icon}
            ...
            <div className="relative">
              <button
                ...
                className={`flex items-center justify-center gap-3 w-full px-4 py-3 rounded-3xl transition-all duration-200 ${
                  showCreateMenu || isCreatePostOpen ? 'bg-white text-black' : 'bg-white/95 text-black/90'
                } shadow-sm`}
              >
            ...
            {user ? (
              <button className="flex items-center gap-4 w-full rounded-[28px] border border-white/10 bg-white/5 px-4 py-3 transition-colors duration-200 hover:bg-white/[0.08] group">
                <div className="w-11 h-11 flex-shrink-0 rounded-full overflow-hidden border border-white/10 group-hover:border-white/20 transition-colors">
                  <img src={user.profilePicture || defaultAvatar} alt={user.fullName} className="w-full h-full object-cover" />
                </div>
